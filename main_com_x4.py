...
#python  /home/Meta_HSI_SR/main_com_x4.py
...
def main():
    ...
    ag.add_argument("--n_scale", type=int, default=4, help="n_scale, default set to 2")
    ...
    ag.add_argument("--model_title", type=str, default="SSPSR-b", help="model_title, default set to model_title")#SSPSR MCnet SSPSR-b GELIN ERCSR
    ...
    OUT_DIR = Path(path)
    np.save(OUT_DIR.joinpath('args.npy'), vars(args))
    ...
    for epoch in range(0, args.epochs):
        ...
        for index, (lrx8, lrx4, lrx2, hr) in loop:
            ...
            loss.backward()
            optimizer.step()
            loop.set_postfix(L1_loss_avg = loss_l1.avg.item(), loss_now = loss.item(),
        ...
        writer.add_scalar('val_psnrx4', np.mean(val_psnrx4), epoch)
        writer.add_scalar('val_samx4', np.mean(val_samx4), epoch)

        if best_sorce["val_psnr"] < np.mean(val_psnrx4):
            best_sorce['val_psnr'] = np.mean(val_psnrx4)
