...
from meta_sr import Adapt_3d,Meta_upsample,bandAttn_Meta_upsample,\
    bandAttn_Meta_upsample_new,bandAttn_Meta_upsample_new_2,Meta_upsample_3d,\
        ...
        Meta_upsample_com_3d_2,Meta_upsample_3d_3
...
class BranchUnit(nn.Module):
    def __init__(self, n_colors, n_feats, n_blocks, act, res_scale, use_up = False, use_tail=True, upmode = 'bandAttn_Meta_upsample', experts = 4, conv=default_conv):
        ...
        self.SA_adapt = Adapt_3d()
    def forward(self, x, n_scale = 1):
        y = self.head(x)
        y = self.body(y)
        # print(y.shape)
        # y = self.upsample(y)
        if self.use_up:
            # y = y.unsqueeze(0).expand(1,64, 256,32,32)
            y = self.upsample(y,n_scale)
            # y = y[:,0,:,:,:]
            # y = y.squeeze(0)
            # print(y.shape)
        if self.tail is not None:
            y = self.tail(y)
            y = self.SA_adapt(y, n_scale)

        return y


class ad_meta_SSPSR(nn.Module):
    ...
    def forward(self, x, n_scale, alpha = .5):
        ...
        for g in range(self.G):
            ...
            if self.shared:
                xi = self.branch(xi, n_scale)
            else:
                xi = self.branch[g](xi, n_scale)
