...
load('D:\FILES_ \HSI_SR\Data\Houston\HoustonU.mat');%% Please down the Chikusei dataset (mat format) from https://www.sal.t.u-tokyo.ac.jp/hyperdata/
...
for i = 1:test_pic_num
    ...
    save(strcat('./test/HoustonU_test_', int2str(i), '.mat'),'test');
end

%% the rest left for training
mkdir ('train');
img = img((test_img_size+1):end,:,:);
save('./train/HoustonU_train.mat', 'img');
